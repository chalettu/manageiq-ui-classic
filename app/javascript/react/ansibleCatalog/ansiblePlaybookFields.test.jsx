...
import ConnectedAnsiblePlaybookFields, { AnsiblePlaybookFields } from './ansiblePlaybookFields';
...
describe('Ansible playbook fields form component', () => {
  ...
  it('should render correctly', () => {
    ...
    exampleStore.dropdowns['provisioning_playbooks'] = [
      { label: 'test', value: 'test'}
    ];
    exampleStore.dropdowns['provisioning_cloudTypes'] = [
      { label: 'test', value: 'test'}
    ];
    store = mockStore({
      ansibleCatalog: {
  ...
  it('should update a list of playbooks', () => {
    ...
    const wrapper = shallow(
      <AnsiblePlaybookFields {...initialProps} data={{ ...reduxStore }} loadPlaybooks={loadPlaybooks} />
    );

    wrapper.instance().updatePlaybookList();
    expect(loadPlaybooks).toHaveBeenCalledWith(1, 1234, 'provisioning');
  });
  it('should update cloud credentials', () => {
    ...
    const loadCloudCredentials = jest.fn(() => {
      return Promise.resolve({});
    });
    const wrapper = shallow(
      <AnsiblePlaybookFields {...initialProps} data={{ ...reduxStore }} loadCloudCredentials={loadCloudCredentials} />
    );

    wrapper.instance().updateCloudCredentialsList('amazon');
    expect(loadCloudCredentials).toHaveBeenCalledWith('provisioning' ,'amazon');
  });
  it('should detect if field values are prepopulated', () => {
    const exampleStore = {...reduxStore};
    initialProps.formValues['provisioning_cloud_type'] = 'test';
    ...
    console.error = jest.fn();
    const wrapper = mount(
      <AnsiblePlaybookFields {...initialProps} data={{ ...reduxStore }}
