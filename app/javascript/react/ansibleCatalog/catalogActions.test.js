...
describe('Catalog Actions', () => {
  ...
  it('displays a list of catalogs', () => {
    const url = '/api/service_catalogs/?attributes=id,name&expand=resources&sort_by=name&sort_order=ascending';
    mockRequest(url, {
      resources,
    });

    return store.dispatch(actions.loadCatalogs()).then(() => {
  ...
  it('Fetches a list of Playbooks', () => {
    ...
    mockRequest(url, { resources });
    return store.dispatch(actions.loadPlaybooks(1, 1, 'provisioning')).then(() => {
  ...
  it('Fetches a list of Cloud credentials', () => {
    ...
    return store.dispatch(actions.loadCloudCredentials('provisioning', 'test')).then(() => {
